        ...
        public void ShowPropertyDetails(Table shownPropertyDetails)
        {
            ...
            actualBookDetails.Should().Match<PROPERTy>(
                ...
                b.PropertyName == expectedPropertyDetails["PropertyName"]
                 ...
                 && b.PropertyType_ID == db.PROPERTY_TYPE.ToList().FirstOrDefault(x => x.CodeType == expectedPropertyDetails["PropertyType"]).ID
                 && b.Price == int.Parse(expectedPropertyDetails["Price"])
                 && b.District_ID == db.DISTRICT_Table.ToList().FirstOrDefault(x => x.DistrictName == expectedPropertyDetails["District"]).ID
                 && b.Ward_ID == db.WARDs.ToList().FirstOrDefault(x => x.WardName == expectedPropertyDetails["Ward_ID"]).ID
                 && b.Street_ID == db.STREETs.ToList().FirstOrDefault(x => x.StreetName == expectedPropertyDetails["Street_ID"]).ID
                 && b.UserID == db.USERs.ToList().FirstOrDefault(x => x.Email == expectedPropertyDetails["UserID"]).ID
                 && b.Status_ID == db.PROJECT_STATUS.ToList().FirstOrDefault(x => x.Status_Name == expectedPropertyDetails["StatusID"]).ID);
        }

        public void OpenPropertyDetails(string propertyId)
        {
            var property = _context.ReferenceBooks.GetById(propertyId);
